...
wire signed [23:0] scale_mult = $signed(x_reg) * $signed(K);
...
wire [3:0] base = {counter, 2'b00};   // 0, 4, 8
...
always @(posedge clk or posedge reset) begin
    if (reset) begin
        ...
        $display("[T=%0t] GG SCALE: x_reg=%0d -> x_reg_next=%0d, y_reg=%0d", $time, x_reg, x_reg_next, y_reg);
...
always @(*) begin
    ...
    case (current_state)
        ...
        ITERATION: begin
            ...
            case (counter)
                2'd0: do_reg_next[3:0]  = {d3_w, d2_w, d1_w, d0_w};
                2'd1: do_reg_next[7:4]  = {d3_w, d2_w, d1_w, d0_w};
                2'd2: do_reg_next[11:8] = {d3_w, d2_w, d1_w, d0_w};
                default: begin end
            endcase
        end

        SCALE: begin
            //x_reg_next = scale_mult >>> K_FRAC;
            x_reg_next = scale_mult[WORD_LENGTH-1+K_FRAC:K_FRAC];
            y_reg_next = 0;
        end
